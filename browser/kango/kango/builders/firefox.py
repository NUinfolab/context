...
class ExtensionBuilder(ExtensionBuilderBase):
    ...
    def _patch_install_manifest(self, manifest_path, locales):
        ...
        with codecs.open(manifest_path, 'w', 'utf-8') as f:
            f.write(doc.toxml().replace('chrome://%s' % KEYWORDS['kango'], 'chrome://%s' % self._info.package_id))

    def _patch_chrome_manifest(self, path):
        with open(path, 'r+') as f:
            content = f.read()
            content = content.replace(KEYWORDS['kango'], self._info.package_id)
            f.truncate(0)
            f.seek(0)
            f.write(content)

    def build(self, out_path):
        self._info.package_id = '%s-' % KEYWORDS['kango'] + self.get_domain_from_id(self._info)
